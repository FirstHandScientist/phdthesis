...
\begin{frame}[label=current]{Insights of RENN}
  Generalized Bethe free energy: diagram illustration

  \emph{i) a RENN requires neither sampling technique nor training data (ground-truth marginal probabilities) in performing inference tasks};\newline
  \emph{ii) a RENN does gradient descent w.r.t. its neural network parameter $\bm{\omega}$ instead of iterative message-passing, and returns approximation of marginal probabilities and partition estimation in one-shot
    iii) No msg. propagation, thus no need of msg. scheduling
    iv) competitive performance and efficiency
  }.
