    ...
    \begin{center}
      {
        \begin{tikzpicture}
          ...
          \node[rnode, below = 0.6cm of inf.west, anchor=west, draw=green] (abp) {$\bullet$ $\alpha$-BP};
...
\begin{frame}[label=current]{Alternative Veiw of BP: $\alpha$-BP}
  Input:
  \begin{columns}
    \column{0.6\textwidth}
...
\begin{frame}[label=current]{Message Propagation Rule}
  Updating message via $\alpha$-BP:
  \begin{align*}
    {m}^{\mathrm{new}}_{ts}(x_s) \propto
    m_{ts}(x_s)^{1-\alpha_{ts}} \bigg[\sum_{x_t} \phi_{ts}(x_t, x_s)^{\alpha_{ts}} {m}_{st}(x_t)^{1-\alpha_{ts}} \underbrace{{\phi}_t(x_t) \prod_{w\in \Nn(t)\backslash s}m_{wt}(x_t)}_{\text{msg. from variable node $t$ to factor $\phi_{st}$}} \bigg].
  \end{align*}
  \centering
  \begin{tikzpicture}
    ...
    \begin{scope}
     ...
     \draw[dashed, ->] (Problem) to (ext);
     \draw[dashed, ->] (ext) to (mmrf);
     \draw[dashed, ->] (mmrf) to (alpha);
...
\begin{frame}[label=current]{Message Propagation Rule}
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}
     ...
     \draw[dashed,->] (Problem) to (pGraph);
     \draw[dashed,->] (pGraph) to (alpha);
     \draw[green,->] (alpha) to (surrogate);
     \draw[green,->] (surrogate) to (sol);
