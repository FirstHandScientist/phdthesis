{ \setbeamercolor{background canvas}{bg=hl_bg}
  ...
  \begin{frame}{What Have We been Talking About?}
    ...
    \begin{tikzpicture}
      ...
      \draw[->] (kl) --node [text width=5cm, midway, above]{\tiny
        \begin{tabular}{c}
        ...
        \end{tabular}}
      (freeEnergy);
      \draw[ ->] (freeEnergy) --node [text width=3cm, midway, left]{\tiny
        \begin{tabular}{c}
          Negative \\
      ...
      \draw[->] (kl) --node [text width=3cm, midway, left]{\tiny
        \begin{tabular}{c}
          $-\mathrm{KL}(q(\bm{x}_U|\bm{x}_O) || p(\bm{x}_U,\bm{x}_O; \bm{\theta})) $
        \end{tabular}}
      (elbo);

      \draw[ ->] (elbo) --node [text width=3cm, midway, above]{\tiny
        \begin{tabular}{c}
      ...
      (llk);
      \draw[ ->] (q-fun) --node [text width=3cm, midway, right]{\tiny
        Below
      }
      (llk);
