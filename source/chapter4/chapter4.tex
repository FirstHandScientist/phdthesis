...
\begin{equation}\label{eq:joint-px}
  p(\bm{x}; \bm{\theta}) = \frac{1}{Z(\bm{\theta})} \prod_{a \in \Ff} \psi_{a}(\bm{x}_{a}; \bm{\theta}_a),
\end{equation}
...
Before giving definition of region graph, we need to define \textit{region} first, which is on the basis of factor graph $\Gg_{\Ff}(\Vv \cup \Ff, \Ee)$ (definition~\ref{chpt2:def:factor-graph}).
\begin{definition}
...
\begin{align}
  m_{P\rightarrow R} \propto \frac{\sum_{\bm{x}_P\backslash
    \bm{x}_R}\prod_{a\in A_P\backslash A_R}\psi_a(\bm{x}_a)\prod_{(I,J)
...
\begin{align}
  b_R(\bm{x}_R) \propto \prod_{a\in A_R} \psi_a(\bm{x}_a) \prod_{P\in
...
\begin{align}\label{apdix:eq:large-region}
  F_{R,L} &=\sum_{R\in \Rr_{L}} c_{R,a}
                              ...
                              \nonumber \\
                            & =\sum_{a\in \Ff} \sum_{\bm{x}_a} b_a(\bm{x}_a)\ln{\frac{b_a(\bm{x}_a)}{\psi_a(\bm{x}_a; \bm{\theta}_a)}}.
\end{align}
...
The representation maping $\bm{f}$ followed by the softmax function in a RENN only needs to directly ouput the beliefs on root regions $\Rr_0$, with the dimension of $|\Rr_0| \times |\Ss(R)| \times K$ (if the number of variable nodes in each root region is the same).
...
\subsubsection{Selection criteria for Root Regions $\Rr_{0}$}
...
Tree-robust root regions can also be constructed for general graphs, which can be seen as an extension from Theorem~\ref{thm:planar-tree-robust} and \ref{thm:complete-tree-robust}. We detail the construction method in the following section.
...
Note that although recent neural message passing methods are also purely neural network based models for inference tasks, these methods still do iterative message propagation analogous to standard BP.
