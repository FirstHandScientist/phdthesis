...
\begin{remark}[Connection to vanilla generative adversarial network]
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \STATE{$l$: the update rate at each iteration, $N$: the batch
      size, and $\bm{\th}_{0}$: the initial parameter for $\bm{g}$.}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \STATE {$l$: the update rate at each iteration, $N$: the batch
      size and $\bm{\th}_{0}$, $\bm{\omega}_0$: the initial parameters for $\bm{g}$ and $\bm{f}$.}
    \WHILE{$\bm{\theta}$ has not converged}
