...
The straightforward issue is the complexity of the graphical representation of dependencies for a dynamic system, which in turn affects model learning and inference. As the dynamic system evolves, the dependencies (which correspond to edges in the graphical model) can be arbitrarily complex since the system states on any two time instants can be dependent if no constraint is enforced. The complexity of the resulting graphical model may become too high for practical usages. Therefore, constraints are required to allow reasonable model learning and state estimations.
...
In this section, we introduce normalizing flows to each state probabilistic model of our GenHMM, which models the conditional probability of observation given a hidden state.
Recall that $\bm{x}\in\mathbb{R}^N$. The subscript of $\bm{x}_t$ is omitted when it does not cause ambiguity.
...
Each generator $\bm{g}_{s,\kappa}$ is realized as a feed-forward neural network.
We define generator $\bm{g}_{s,\kappa}$ as an $L$-layer flow model and formulate its mapping by layer-wise concatenation
\begin{equation}
...
\begin{align}\label{eq:obj-q-gen-mix}
  &\Qq(\bm{\Theta}; \bm{H}^{\mathrm{old}}) \nonumber \\
  =& \frac{1}{R}\sum_{r=1}^{R}\hspace{-1pt} \sum_{t=1}^{{T}^{r}}\hspace{-1pt} \sum_{\smtr{s}{t}{r}=1}^{|\Ss|} \hspace{-1pt} \sum_{\smtr{\kappa}{t}{r}=1}^{K}p(\smtr{s}{t}{r}| \ubmr{x}{r}; \bm{H}^{\mathrm{old}})p(\smtr{\kappa}{t}{r}|\smtr{s}{t}{r}, \ubmr{x}{r}; \bm{H}^{\mathrm{old}}) \nonumber\\
...
To solve problem in \eqref{opm:pi}, we formulate its Lagrange function as
\begin{equation}
...
\end{equation}
where $p(\smtr{s}{1}{r}=i | \ubmr{x}{r}; \bm{H}^{\mathrm{old}})$ is obtained from forward-backward algorithm.
...
\begin{align}
  ...
  =&\frac{1}{R} \sum_{r=1}^{R} \sum_{\ubmr{s}{r}}{p(\ubmr{s}{r}| \ubmr{x}{r}; \bm{H}^{\mathrm{old}})} \sum_{t=1}^{T^{r}-1}\log\,p(\smtr{s}{t+1}{r}|\smtr{s}{t}{r}; \bm{H}) \nonumber \\
  =&\frac{1}{R} \sum_{r=1}^{R} \hspace{-0.1cm}\sum_{t=1}^{T^{r}-1}\hspace{-0.1cm} \sum_{\smtr{s}{t}{r}=1}^{|\Ss|}\hspace{-0.05cm}\sum_{\smtr{s}{t+1}{r}=1}^{|\Ss|}\hspace{-0.2cm}{p(\smtr{s}{t}{r}, \smtr{s}{t+1}{r}| \ubmr{x}{r};\hspace{-0.05cm} \bm{H}^{\mathrm{old}})} \log\,p(\smtr{s}{t+1}{r}|\smtr{s}{t}{r}; \bm{H}).
\end{align}
...
\end{equation}
with $p(\smtr{s}{t}{r}=i, \smtr{s}{t+1}{r}=j| \ubmr{x}{r}; \bm{H}^{\mathrm{old}})$ computed from forward-backward algorithm.
\subsection{On Convergence of GenHMM}
In pursuit of representing a dataset by GenHMM,  we are interested in knowing if the learning solution discussed in Section~\ref{subsec:optmGenHMM} converges. The properties of GenHMM's convergence are analyzed as follows.
...
\begin{proof}
  ...
  \begin{align*}
    &\EE_{\hat{p}(\ubm{x}),p(\ubm{s},\ubm{\kappa}| \ubm{x}; \bm{H}^{\mathrm{old}})}\left[ \log\,\frac{p(\ubm{x}, \ubm{s}, \ubm{\kappa}; \bm{H}^{\mathrm{new}})}{p(\ubm{s}, \ubm{\kappa}|\ubm{x}; \bm{H}^{\mathrm{old}})}\right] \nonumber \\
    = &\Qq(\bm{H}^{\mathrm{new}}; \bm{H}^{\mathrm{old}}) - \EE_{\hat{p}(\ubm{x}),p(\ubm{s},\ubm{\kappa}| \ubm{x}; \bm{H}^{\mathrm{old}})}\left[\log{p(\ubm{s}, \ubm{\kappa}|\ubm{x}; \bm{H}^{\mathrm{old}})}\right].
  \end{align*}
  ...
  \end{equation*}
  With the above inequality, and the fact that $\EE_{\hat{p}(\ubm{x}),p(\ubm{s},\ubm{\kappa}| \ubm{x}; \bm{H}^{\mathrm{old}})}\left[\log{p(\ubm{s}, \ubm{\kappa}|\ubm{x}; \bm{H}^{\mathrm{old}})}\right]$ is independent of $\bm{H}^{\mathrm{new}}$, we have the inequality 
  \begin{align*}
    &\EE_{\hat{p}(\ubm{x}),p(\ubm{s},\ubm{\kappa}| \ubm{x}; \bm{H}^{\mathrm{old}})}\left[ \log\,\frac{p(\ubm{x}, \ubm{s}, \ubm{\kappa}; \bm{H}^{\mathrm{new}})}{p(\ubm{s}, \ubm{\kappa}|\ubm{x}; \bm{H}^{\mathrm{old}})}\right] 
...
\begin{algorithm}[!tp]
  ...
  \begin{algorithmic}[1]
    \STATE {\bfseries Input:}{
      Empirical distribution $\hat{p}(\ubm{x})$ of dataset}\\
    ...
    \STATE Set learning rate $\eta$, neural network optimization steps, $N$ per EM step
...
We summarize the learning solution in Section~\ref{subsec:optmGenHMM} in the pseudocode as shown in Algorithm~\ref{algo:genhmm}. We use the Adam optimizer \cite{DBLP:journals/corr/KingmaB14} for optimization with regard to the parameters of generators in GenHMM. As shown in Algorithm~\ref{algo:genhmm}, the batch-size stochastic gradient descent can be naturally embedded into the learning algorithm of GenHMM.
...
The TIMIT dataset consists of $5300$ phoneme-labeled speech utterances which are partitioned into two sets: {a train set consists of $4620$ utterance, and a test set consists of $1680$ utterances.} There are totally $61$ different types of phonemes in TIMIT.
We performed experiments in two cases: i) full $61$-phoneme classification case; ii) $39$-phoneme classification case, where $61$ phonemes are folded onto $39$ phonemes as described in \cite{Perdigao11}.
...
Our experiments are performed for: i) standard classification tasks (Tables~\ref{tab:acc-classification39f_a}, \ref{tab:acc-classification39f_b}, \ref{tab:acc-classification13f_a}, and \ref{tab:acc-classification13f_b}), ii) classification under noise perturbation (Tables~\ref{tab:acc-classification39f_noise_snr} and \ref{tab:acc-classification39f_noise_type}). The criterion used to report the results includes accuracy, precision and F1 scores.
...
The exact number of states is decided by computing the average length of MFCC frames per phoneme in training dataset, and clipping the average length into $\left\{ 3,4,5 \right\}$.
...
The classes prior probabilities $p(\bm{H}_{i}),~i=0, 1$ are infered from the training dataset. We use \textit{dGenHMM} to denote that GenHMM is further learned by discriminative training with objective \eqref{chpt7:eq:discriminative}, i.e., a fine-tuning phase, after default generative training phase.
...
The sequential signals used are the Respiratory Frequency (RF), the RR-interval (RRi, beat to beat interval) in electrocardiography, and the blood oxygen saturation level ($SpO_2$).
...
Similar to heart rate observation system (HeRO) \cite{hicksHeartRateObservation2013, griffinHeartRateCharacteristics2005}, a logistic regression method using RRi, we aim at detecting septic events earlier than clinical suspicion of sepsis.
...
Time frames which are labeled neither $0$ nor $1$ were discarded.
...
Pulse oximetry predictive score (POPS) \cite{sullivanEarlyPulseOximetry2018} uses mean, standard deviation, skewness, kurtosis, and min-max cross correlation between RRi and $SpO_2$ to compute a risk score. POPS is also a logistic regression method. 
...
Given our limited input dimension, $\bm{m}_a$ and $\bm{m}_b$ are $3$-layer feed-forward neural networks.
The chain-length in the coupling layers of each flow was varied between $4$ and $8$.
...
This lead to $2361 \pm 353$ time series in the training sets and $1140 \pm 353$ time series in the testing set.
We used a different set of input features to test our models in different conditions.
HMMs are trained on raw time series only, and on raw time series with first and second order derivatives.
...
Extreme learning machines (ELM) and support vector machine (SVM) with a Gaussian kernel trained on the raw time series are also added into the comparison.
...
\multicolumn{4}{l}{Using feature extraction}    \\ \hline
Logistic Regression	 & & POPS & 0.54$\pm$0.01				  \\
& & HeRO			&   0.57$\pm$0.04\\   \hline
\multicolumn{3}{l}{Using raw sequential input}    \\    \hline  
SVM      & &                       &   0.60 $\pm$ 0.04   			\\
ELM		& &						&   0.60 $\pm$ 0.03   				\\ 
...
When the number of states increases to $\abs{\Ss}=6$ and $\abs{\Ss}=9$, GMM-HMM reaches $68\%$ and $69\%$ which outperforms GenHMM, and dGenHMM.
...
GenHMM and dGenHMM also reach their highest performance with $69\%$ at $\abs{\Ss}=3$ and $72\%$ at $\abs{\Ss}=6$.
