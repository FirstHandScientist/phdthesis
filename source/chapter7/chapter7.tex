...
\begin{remark}
  ...
  In a nutshell, omitting observed variables and their edges, the remaining variables and their edges form an MRF in a CRF. This difference also affects the model training. An HMM is a generative model and models $p(\ubar{\bm{x}}, \ubar{{s}})$ jointly. Its training target is to maximize the joint probability. In contrast, a CRF models a conditional distribution $p(\ubar{{s}}|\ubar{\bm{x}})$ and is learned via discriminative training. CRFs are popular applied to structure prediction such as natural language processing, computer vision, bioinformatics, etc.
...
Recall that $\bm{x}\in\mathbb{R}^N$. The subscript is $\bm{x}_t$ omitted when it does not cause ambiguity.
...
As we will detail in the next section, what is needed to formulate the problem, are actually the $p(s| \ubm{x}; \bm{H})$ and $p(s, \kappa| \ubm{x}; \bm{H})$. For the joint posterior $p(s, \kappa| \ubm{x}; \bm{H})$, it can be computed by the Bayes rule when posterior of hidden state is available.
  ...
  The considered domain $\Ss$ of hidden state $s$ here has small cardinality. If a hidden variable itself is structured and the cardinality of its domain is large, the exact posterior probability computation of the hidden variable by forward-backward is not longer feasible. In this case, approximate inference methods as discussed in Part~\ref{part:inference} may be applied. See \cite{Pelizzola_2013,NIPS2018_8013} for related works that explore this problem. 
\end{remark}
