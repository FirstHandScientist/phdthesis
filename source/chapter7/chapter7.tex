...
The straightforward issue is the complexity of the graphical representation of dependencies for a dynamic system, which in turn affects model learning and inference. As the dynamic system evolves, the dependencies (which correspond to edges in the graphical model) can be arbitrarily complex since the system states on any two time instants can be dependent if no constraint is enforced. The complexity of the resulting graphical model can be too high for practical usages. Therefore, constraints are required to allow reasonable model learning and state estimations.
...
We summarize the learning solution in Section~\ref{subsec:optmGenHMM} in the pseudocode as shown in Algorithm~\ref{algo:genhmm}. We use the Adam optimizer \cite{DBLP:journals/corr/KingmaB14} for optimization with regard to the parameters of generators in GenHMM. As shown from line $6$ to $12$ in Algorithm~\ref{algo:genhmm}, the batch-size stochastic gradient descent can be naturally embedded into the learning algorithm of GenHMM.
...
Given our limited input dimension, the number of the mixture components was fixed to $3$.
